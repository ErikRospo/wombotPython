...
let media_store_rest = new Rest("app.wombo.art", 100);
module.exports = async function task(
  ...
  let {
    ...
    reference_image_dir = "./refrences/"
  } = settings;
  if (final || inter) mkdirp(download_dir);
  if (reference_image) mkdirp(reference_image_dir);
  ...
  let image_weight = "HIGH";
  let media_store_uuid = "";
  if (reference_image) {
    download(
      reference_image,
      reference_image_dir + reference_image.split("/").pop()
    ).then(async () => {
      let image_uri = readFileSync(
        reference_image_dir + reference_image.split("/").pop(),
        "base64"
      );
      let media_suffix = reference_image.split("/").pop().split(".")[1];
      media_store_uuid = await media_store_rest
        .post(
          "/api/mediastore",
          { image: image_uri, media_suffix: media_suffix, num_uploads: 1 },
          "DELETE",
          {
            ...
            "Content-Length": image_uri.length,
            path: "/api/mediastore",
        ...
        .then(res => {
          ...
          return res.json().mediastore_uid;
        });
      console.log(media_store_uuid);
      // decodeURI();
    });
  }
  ...
  if (!reference_image) {
    try {
      task = await paint_rest.options(task_path, "PUT").then(() =>
        paint_rest.put(task_path, {
          input_spec: {
            display_freq: 10,
            prompt,
            style: +style
      ...
      task = await paint_rest.options(task_path, "PUT").then(() =>
        paint_rest.put(task_path, {
          input_spec: {
            ...
            input_image: {
              weight: image_weight,
              mediastore_id: media_store_uuid
            },
            style: +style
          }
        })
