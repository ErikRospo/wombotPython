...
class Rest:
    def __init__(self,hostname,delay=250,cookies_enabled=True,supports_https=False,default_port=443):
    ...
    def get_cookies(self):
        if (not self.cookies_enabled): return ""
        return "; ".join(["{}={}".format(name, value) for name, value in self.cookies.items()])
    ...
    def post(self,path, data={}, request_headers={}, method = "POST",port=-1):
        ...
        res=requests.request(stream=False,method=method,url=self.hostname+path+":"+str(port),headers=headers,cookies=self.get_cookies(),data=post_data)
    ...
    def get(self,path, request_headers={}, method = "GET",port=-1):
        ...
        res=requests.request(stream=False,method=method,url=self.hostname+path+":"+str(port),headers=headers,cookies=self.get_cookies())
        self.handle_cookies(res.headers["set-cookie"])
        return res.json()
    ...
    def options(self,path, request_method,port=-1):
        ...
        return self.get(path, method="OPTIONS", request_headers=request_headers,port=port)
...
if __name__=="__main__":
    ...
    print("options done")
    print("post starting")
    a.post("/api/tasks",data=json.loads("{premium:false}"),request_headers={})
    print("post done")